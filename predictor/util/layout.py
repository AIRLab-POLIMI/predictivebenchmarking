...
def start_canny_ed_hough(thresh1,parametri_obj):
	...
	walls = cv2.HoughLinesP(cannyEdges,parametri_obj.rho,parametri_obj.theta,parametri_obj.thresholdHough,parametri_obj.minLineLength,parametri_obj.maxLineGap)
...
def contorno_esterno(img_rgb,parametri_obj, path_obj):
	...
	cannyEdges = cv2.Canny(img_rgb,parametri_obj.minVal,parametri_obj.maxVal,apertureSize=parametri_obj.apertureSize)
	#t=1 #threshold di hough
	#m=20 #maxLineGap di hough
	hough_contorni, contours = im.trova_contorno(parametri_obj.t,parametri_obj.m,cannyEdges, path_obj.metricMap)
