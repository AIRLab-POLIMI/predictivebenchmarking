...
def start_main(minLateralSeparation, minVal, maxVal, rho, theta, thresholdHough, minLineLength, maxLineGap, eps, minPts, h, minOffset, apertureSize, t, m, metricMap,outLayout_pickle_path, outTopologicalGraph_pickle_path, SAVE_FOLDER, LOGFILE):

	#----------------------------LAYOUT DELLE STANZE-----------------------------------
	#layout delle stanze
	(stanze, clustersCelle, estremi, colori, spazi) = lay.get_layout(metricMap, minVal, maxVal, rho, theta, thresholdHough, minLineLength, maxLineGap, eps, minPts, h, minOffset, minLateralSeparation,apertureSize, t, m, SAVE_FOLDER)
	for s in stanze:
		print str(s.area*0.025*0.025)+" m^2"
...
def main():
	...
	PKL_FOLDER = os.path.join( path_par.INFOLDERS, DATASET_Immagini)#in questo momento sto usando le immagini nella cartella old
	...
	with open(SAVE_LOGFILE,'w+') as LOGFILE:
		print "AZIONE", params.AZIONE
		print >>LOGFILE, "AZIONE", params.AZIONE
		...
		if params.AZIONE == 'batch':
			for metricMap in glob.iglob(os.path.join(PKL_FOLDER, "*.png")):
				start_main(par.minLateralSeparation, par.minVal, par.maxVal, par.rho, par.theta, par.thresholdHough, par.minLineLength, par.maxLineGap, par.eps, par.minPts, par.h, par.minOffset, par.apertureSize, par.t, par.m, metricMap, path_par.outLayout_pickle_path, path_par.outTopologicalGraph_pickle_path, SAVE_FOLDER, LOGFILE)
			LOGFILE.flush()
		elif params.AZIONE == 'mappa_singola':
			#immagine della mappa metrica presa singolarmente
			metricMap = './battle_creekhs_2_updated.png'
			start_main(par.minLateralSeparation, par.minVal, par.maxVal, par.rho, par.theta, par.thresholdHough, par.minLineLength, par.maxLineGap, par.eps, par.minPts, par.h, par.minOffset, par.apertureSize, par.t, par.m, metricMap, path_par.outLayout_pickle_path, path_par.outTopologicalGraph_pickle_path, SAVE_FOLDER, LOGFILE)
			LOGFILE.flush()
